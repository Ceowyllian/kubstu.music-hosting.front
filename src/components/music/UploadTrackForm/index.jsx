...
function UploadTrackForm() {
  ...
  const { getTouchedValues, handleSubmit, reset, isTouched } = useForm(
    fields,
    async (e) => {
      ...
      <Card.Body>
        <Form>
          <Row>
            ...
            <Col>
              <Card.Img src={fields.image.fileUrl} fluid />
            </Col>
          </Row>
        </Form>
      </Card.Body>
      <Card.Footer>
        <Button size={'sm'} disabled={!isTouched} onClick={handleSubmit}>
          Upload
        </Button>
        <Button
